<!DOCTYPE html>
<html>
  <head>
    <header>
    <title>Colegio de educación integral A-B-C</title>
    <link rel="stylesheet" 
    href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" 
    integrity="sha384-JcKb8q3iqJ61gNV9KGb8thSsNjpSL0n8PARn9HuZOnIxN0hoP+VmmDGMN5t9UJ0Z" 
    crossorigin="anonymous">
    <link href="https://fonts.googleapis.com/css?family=Roboto&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="css/index.css">
    <link rel="stylesheet" href="style.css">
    <script src="javascript/index.js"></script>
</head>
  <body>
  <nav class="navbar navbar-expand-sm navbar-light">
      <div class="container">
      <a class="navbar-brand" href="#">Colegio abc</a>
      <button class="navbar-toggler collapse" type="button" data-bs-toggle="collapse" data-bs-target="#collapsibleNavId" aria-controls="collapsibleNavId"
        aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
      </button>
      <div class="collapse navbar-collapse" id="collapsibleNavId">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          <li class="nav-item">
            <a class="nav-link active" href="index.php" aria-current="page">Inicio <span class="visually-hidden"> (actual)</span></a>
          </li>
          <li class="nav-item"><a class="nav-link" aria-current="page" href="acerca.html">Acerca</a></li>
          <li class="nav-item"><a class="nav-link" aria-current="page" href="programas.html">Pre-Inscripción</a></li>
          <li class="nav-item"><a class="nav-link" aria-current="page" href="">Noticias</a></li>
          <li class="nav-item"><a class="nav-link" aria-current="page" href="contacto.html">Contacto</a></li>
        </ul>
        <div class="dropdown">
          <button onclick="myFunction()" class="dropbtn">Iniciar sesion como:</button>
            <div id="myDropdown" class="dropdown-content">
              <a href="ESTUDIANTE.php">ESTUDIANTE</a>
              <a href="DOCENTE.php">DOCENTE</a>
              <a href="ACUDIENTE.php">PADRE DE FAMILIA O ACUDIENTE</a>
              <a href="admin.php">ADMINISTRADOR</a>
            </div>
        </div>
      </div>
    </div>
  </nav>
  
  </header>
      <h1>Colegio de educación integral A-B-C</h1>
  
    
<!-- <img id="escudo" src="data:image/png;base64," alt="escudo"> -->
    

<section>
<div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
  <div class="carousel-inner">
    <div class="carousel-item active">
      <img style="max-height: 700px;" class="d-block w-100" src="https://www.archwayfacilities.co.uk/wp-content/uploads/2021/05/EF94B9.jpg" alt="First slide">
    </div>
    <div class="carousel-item">
      <img style="max-height: 700px;" class="d-block w-100" src="https://educationlifeskills.com/storage/2019/02/AdobeStock_218308627-e1549951107494.jpeg" alt="Second slide">
    </div>
    <div class="carousel-item">
      <img style="max-height: 700px;" class="d-block w-100" src="https://miro.medium.com/v2/resize:fit:1200/1*CXcWruRNtDRtDS1YFXzPbA.jpeg" alt="Third slide">
    </div>
  </div>
  <a class="carousel-control-prev" href="#carouselExampleControls" role="button" data-slide="prev">
    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
    <span class="sr-only">Previous</span>
  </a>
  <a class="carousel-control-next" href="#carouselExampleControls" role="button" data-slide="next">
    <span class="carousel-control-next-icon" aria-hidden="true"></span>
    <span class="sr-only">Next</span>
  </a>
</div>
</div>
</section>

<section>
<div class="container-section">
  <div>
  <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Laudantium quas nemo eum consectetur doloribus rerum voluptate dignissimos quidem ducimus ea rem labore quae, consequatur fugit cumque sapiente. Corporis, explicabo quaerat?</p> 
  </div>
  <div>
  <img style="max-height: 500px;" src="https://youarecurrent.com/wp-content/uploads/2014/04/BoyswithRobotPicture-1024x682.jpg" alt="primaria">
  </div>
</div>
</section>



<section>
<div class="container-section2">
  <div>
  <img src="https://yandex.com/images/search?img_url=http%3A%2F%2Fmedia.baamboozle.com%2Fuploads%2Fimages%2F1069605%2F1670228356_392503.jpeg&lr=21174&pos=37&rpt=simage&text=profesiones" alt="">
  </div>
  <div>
  <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempore delectus recusandae, nisi, officiis qui ipsam suscipit et, pariatur labore dicta ratione dignissimos dolor saepe. Impedit reprehenderit inventore dolores nulla enim.</p> 
  </div>
</div>
</section>

    <footer>
      <p>Copyright © 2023 Colegio de educación integral A-B-C</p>
    </footer>
  </body>

</html>